...
def xoa_ghe_ngoi():
    ...
    try:
        cursor.execute("delete from ghe_ngoi where id = %s", (id,))
        conn.commit()
...
root = tk.Tk()
root.title("Xóa Ghế Ngồi")
root.geometry("400x300")

tk.Label(root, text="Xóa Ghế Ngồi", font=('Arial', 14, 'bold')).grid(row=0, column=0, columnspan=2, pady=10)
...
btn_edit = tk.Button(root, text="Xóa", width=15, font=('Arial', 12, 'bold'), bg="#4CAF50", fg="white", command=xoa_ghe_ngoi)
