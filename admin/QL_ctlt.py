...
def hien_thi(listbox, query=""):
    ...
    if query:  # If there's a search query
        cursor.execute("SELECT * FROM chi_tiet_lich_trinh WHERE id_tau LIKE %s OR id_lich LIKE %s", (f"%{query}%", f"%{query}%"))
    else:
        cursor.execute("SELECT * FROM chi_tiet_lich_trinh")

    rows = cursor.fetchall()
    for row in rows:
        listbox.insert("", "end", values=(row['id_tau'], row['id_lich']))
...
def them_ctlt(listbox):
    def save_new_seat():
        idtau = tinhtrang_entry.get()
        idlich = idtoa_entry.get()

        if not idtau or not idlich:
            messagebox.showwarning("Cảnh báo", "Vui lòng nhập đầy đủ thông tin!")
            return

        try:
            cursor.execute("INSERT INTO chi_tiet_lich_trinh (id_tau, id_lich) VALUES (%s, %s)", (idtau, idlich))
            conn.commit()
    ...
    tk.Label(root, text="ID tàu:", font=('Arial', 12)).grid(row=2, column=0, padx=10, pady=10, sticky="w")
    tinhtrang_entry = tk.Entry(root, font=('Arial', 12))
    tinhtrang_entry.grid(row=2, column=1, padx=10, pady=10, sticky="ew")

    tk.Label(root, text="ID lịch:", font=('Arial', 12)).grid(row=3, column=0, padx=10, pady=10, sticky="w")
...
def sua(listbox):
    ...
    if not selected_item:
        messagebox.showwarning("Cảnh báo", "Vui lòng chọn một ctlt để sửa!")
        return

    selected_data = listbox.item(selected_item[0], "values")
    tinh_trang = selected_data[0]
    id_toa = selected_data[1]
    ...
    def save_changes():
        ...
        try:
            cursor.execute("UPDATE chi_tiet_lich_trinh SET id_tau = %s, id_lich = %s WHERE id_tau = %s and id_lich = %s",
                           (new_tinh_trang, new_id_toa,tinh_trang))
            conn.commit()
    ...
    root = tk.Tk()
    root.title("Sửa chi tiết lịch trình")
    root.geometry("400x300")
...
def xoa(listbox):
    ...
    if not selected_item:
        messagebox.showwarning("Cảnh báo", "Vui lòng chọn một ctlt để xóa!")
        return

    selected_data = listbox.item(selected_item[0], "values")
    id_ghe = selected_data[0]

    confirm = messagebox.askyesno("Xác nhận", f"Bạn có chắc chắn muốn xóa ctlt với ID {id_ghe}?")
    if confirm:
        try:
            cursor.execute("DELETE FROM chi_tiet_lich_trinh WHERE id_tau = %s", (id_ghe,))
            conn.commit()
...
def show_ql_ctlt(main_frame, show_home_func):
    for widget in main_frame.winfo_children():
        widget.destroy()

    tk.Label(main_frame, text="Quản lý chi tiết lịch trình", font=('Arial', 14, 'bold')).pack(pady=10)
    ...
    listbox = ttk.Treeview(main_frame, columns=("ID tàu", "ID lịch"), show="headings")
    for col in ("ID tàu", "ID lịch"):
    ...
    tk.Button(btn_frame, text="Thêm", width=10, bg="#4CAF50", fg="white", command=lambda: them_ctlt(listbox)).grid(row=0, column=0, padx=5, pady=5)
