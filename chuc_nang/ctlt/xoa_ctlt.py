...
def xoa():
    ...
    try:
        cursor.execute("delete from xoa where id_tau = %s", (id_tau,))
        conn.commit()
...
root = tk.Tk()
root.title("Xóa Chi Tiết Lịch Trình")
root.geometry("400x300")

tk.Label(root, text="Xóa Chi Tiết Lịch Trình", font=('Arial', 14, 'bold')).grid(row=0, column=0, columnspan=2, pady=10)
...
btn_delete = tk.Button(root, text="Xóa", width=15, font=('Arial', 12, 'bold'), bg="#f44336", fg="white", command=xoa)
