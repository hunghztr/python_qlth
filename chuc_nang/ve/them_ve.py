...
def them():
    ...
    if not giatien or not tentoa or not tentau or not kh or not ghe:
        messagebox.showwarning("Cảnh báo", "Vui lòng nhập đầy đủ thông tin!")
        return

    try:
        cursor.execute("INSERT INTO ve_tau (gia_tien, ten_toa, ten_tau, id_kh, id_ghe_ngoi) VALUES (%s, %s, %s, %s, %s)",
                       (giatien, tentoa, tentau, kh, ghe))
        conn.commit()
...
tk.Label(root, text="Thêm Vé", font=('Arial', 14, 'bold')).grid(row=0, column=0, columnspan=2, pady=10)
