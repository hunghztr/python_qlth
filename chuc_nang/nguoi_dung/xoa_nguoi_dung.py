...
def xoa_nguoi_dung():
    ...
    try:
        cursor.execute("delete from nguoi_dung where id = %s", (id,))
        conn.commit()
...
root = tk.Tk()
root.title("Xóa Người Dùng")
root.geometry("400x350")

tk.Label(root, text="Xóa Người Dùng", font=('Arial', 14, 'bold')).grid(row=0, column=0, columnspan=2, pady=10)
...
btn_edit = tk.Button(root, text="Xóa", width=15, font=('Arial', 12, 'bold'), bg="#4CAF50", fg="white", command=xoa_nguoi_dung)
