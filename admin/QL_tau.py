...
def hien_thi(listbox, query=""):
    ...
    if query:  # If there's a search query
        cursor.execute("SELECT * FROM tau WHERE id LIKE %s OR ten LIKE %s OR so_toa LIKE %s", (f"%{query}%", f"%{query}%",f"%{query}%" ))
    else:
        cursor.execute("SELECT * FROM tau")

    rows = cursor.fetchall()
    for row in rows:
        listbox.insert("", "end", values=(row['id'], row['ten'], row['so_toa']))  # Insert data into Treeview
...
def them_tau(listbox):
    def save_new_seat():
        ...
        try:
            cursor.execute("INSERT INTO tau (ten, so_toa) VALUES (%s, %s)", (tinh_trang, id_toa))
            conn.commit()
...
def sua(listbox):
    ...
    def save_changes():
        ...
        try:
            cursor.execute("UPDATE tau SET ten = %s, so_toa = %s WHERE id = %s",
...
def xoa(listbox):
    ...
    if not selected_item:
        messagebox.showwarning("Cảnh báo", "Vui lòng chọn một tàu để xóa!")
        return

    selected_data = listbox.item(selected_item[0], "values")
    id_ghe = selected_data[0]

    confirm = messagebox.askyesno("Xác nhận", f"Bạn có chắc chắn muốn xóa tàu với ID {id_ghe}?")
    if confirm:
        try:
            cursor.execute("DELETE FROM tau WHERE id = %s", (id_ghe,))
            conn.commit()
...
def show_ql_tau(main_frame, show_home_func):
    for widget in main_frame.winfo_children():
        widget.destroy()

    tk.Label(main_frame, text="Quản lý tàu", font=('Arial', 14, 'bold')).pack(pady=10)
    ...
    listbox = ttk.Treeview(main_frame, columns=("ID", "Tên", "Số toa"), show="headings")
    for col in ("ID", "Tên", "Số toa"):
    ...
    tk.Button(btn_frame, text="Thêm", width=10, bg="#4CAF50", fg="white", command=lambda: them_tau(listbox)).grid(row=0, column=0, padx=5, pady=5)
